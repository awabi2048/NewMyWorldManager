...
class WorldSettingsListener : Listener {
        ...
        when (session.action) {
            SettingsAction.MANAGE_MEMBERS -> {
                ...
                if (type == ItemTag.TYPE_GUI_NAV_NEXT || type == ItemTag.TYPE_GUI_NAV_PREV) {
                    ...
                    lore.forEach { line ->
                        ...
                        if (plainLine.startsWith("PAGE_TARGET: ")) {
                            val targetPage =
                                    plainLine.removePrefix("PAGE_TARGET: ").trim().toIntOrNull()
                                            ?: return@forEach
                            plugin.soundManager.playClickSound(player, item, "world_settings")
                ...
                if (type == ItemTag.TYPE_GUI_MEMBER_INVITE) {
                    ...
                    player.sendMessage(
                            plugin.languageManager.getMessage("messages.member_invite_input") +
                                    " " +
                                    cancelInfo
                    )
                    return
                ...
                if (type == ItemTag.TYPE_GUI_MEMBER_ITEM) {
                    ...
                    if (memberId != null && memberId != player.uniqueId) {
                        ...
                        if (event.isShiftClick && event.isRightClick) {
                            ...
                            val currentCounts =
                                    plugin.worldConfigRepository.findAll().count {
                                        it.owner == memberId
                                    }
            ...
            SettingsAction.MEMBER_REMOVE_CONFIRM -> {
                ...
                if (type == ItemTag.TYPE_GUI_CANCEL) {
                    ...
                    if (memberId != null) {
                        ...
                        player.sendMessage(
                                plugin.languageManager.getMessage("messages.member_deleted")
                        )
                    ...
                    if (visitor != null && visitor.world.name == worldFolderName) {
                        ...
                        val yaw =
                                config.getDouble(
                                                ...
                                                evacWorld.spawnLocation.yaw.toDouble()
                                        )
                                        .toFloat()
                        val pitch =
                                config.getDouble(
                                                "evacuation_location.pitch",
                                                evacWorld.spawnLocation.pitch.toDouble()
                                        )
                                        .toFloat()
                        ...
                        visitor.sendMessage(
                                plugin.languageManager.getMessage(visitor, "messages.kicked")
                        )
                        player.sendMessage(
                                plugin.languageManager.getMessage(
                                        player,
            ...
            SettingsAction.EXPAND_SELECT_METHOD -> {
                ...
                if (type == ItemTag.TYPE_GUI_SETTING_EXPAND) {
                    ...
                    player.closeInventory()
                    player.sendMessage(
                            plugin.languageManager.getMessage("messages.expand_direction_prompt")
                    )
                } else if (type == ItemTag.TYPE_GUI_CANCEL) {
                    handleCommandCancel()
                }
            ...
            SettingsAction.EXPAND_CONFIRM -> {
                ...
                if (type == ItemTag.TYPE_GUI_CANCEL ||
                    ...
                    val task =
                            Bukkit.getScheduler()
                                    .runTaskLater(
                                            ...
                                            Runnable {
                                                val pending =
                                                        pendingExpansions.remove(player.uniqueId)
                                                                ?: return@Runnable
                    ...
                    pendingExpansions[player.uniqueId] =
                            PendingExpansion(worldData, cost, session.expansionDirection, task)
                    plugin.settingsSessionManager.endSession(player)
                }
            }
            SettingsAction.VIEW_SETTINGS, SettingsAction.SELECT_ICON -> {
                ...
                if (session.action == SettingsAction.SELECT_ICON) {
                    ...
                    if (clickedItem.type != Material.AIR &&
                                    !ItemTag.isType(clickedItem, ItemTag.TYPE_GUI_DECORATION)
                    ) {
                        worldData.icon = clickedItem.type
                        plugin.worldConfigRepository.save(worldData)
                        val itemName =
                                clickedItem.displayName().decoration(TextDecoration.ITALIC, false)
                        player.sendMessage(
                                plugin.languageManager.getMessage(
                                        player,
                ...
                when (itemTag) {
                    ItemTag.TYPE_GUI_RETURN -> {
                        me.awabi2048.myworldmanager.util.GuiHelper.handleReturnClick(
                                plugin,
                                player,
                                clickedItem
                        )
                    ...
                    ItemTag.TYPE_GUI_SETTING_EXPAND -> {
                        ...
                        val cost =
                                if (config.contains("expansion.costs.$targetLevel")) {
                    ...
                    ItemTag.TYPE_GUI_SETTING_VISITOR -> {
                        plugin.soundManager.playClickSound(player, clickedItem, "world_settings")
                        val worldFolderName =
                                worldData.customWorldName ?: "my_world.${worldData.uuid}"
                    ...
                    ItemTag.TYPE_GUI_SETTING_ANNOUNCEMENT -> {
                        ...
                        if (event.isRightClick) {
                            ...
                            plugin.worldConfigRepository.save(worldData)
                            player.sendMessage(
                                    plugin.languageManager.getMessage("messages.announcement_reset")
                            )
                            plugin.worldSettingsGui.open(player, worldData)
                        } else {
                            val cancelWord =
                    ...
                    ItemTag.TYPE_GUI_SETTING_PORTALS -> {
                        ...
                        val hasPortals =
                                plugin.portalRepository.findAll().any { it.worldName == worldName }
                ...
                if (type == ItemTag.TYPE_PORTAL) {
                    ...
                    if (event.isRightClick) {
                        ...
                        if (portal != null) {
                            ...
                            val returnItem =
                                    me.awabi2048.myworldmanager.util.PortalItemUtil
                                            .createBasePortalItem(lang, player)
                            if (portal.worldUuid != null) {
                                val destData =
                                        plugin.worldConfigRepository.findByUuid(portal.worldUuid!!)
                                me.awabi2048.myworldmanager.util.PortalItemUtil.bindWorld(
                                        returnItem,
                                        portal.worldUuid!!,
                                ...
                                me.awabi2048.myworldmanager.util.PortalItemUtil.bindExternalWorld(
                                        returnItem,
                                        portal.targetWorldName!!,
                                        displayName,
                                        lang,
            ...
            SettingsAction.DELETE_WORLD_CONFIRM_FINAL -> {
                ...
                if (type == ItemTag.TYPE_GUI_CANCEL) {
                    ...
                    plugin.worldService.deleteWorld(worldData.uuid).thenAccept { success: Boolean ->
                        Bukkit.getScheduler()
                                .runTask(
                                        plugin,
                                        Runnable {
            ...
            SettingsAction.UNARCHIVE_CONFIRM -> {
                ...
                if (type == ItemTag.TYPE_GUI_CANCEL) {
                    ...
                    player.closeInventory()
                    player.sendMessage(
                            plugin.languageManager.getMessage(player, "messages.unarchive_start")
                    )
    ...
    fun onChat(event: AsyncChatEvent) {
        ...
        if (worldData == null && session.action != SettingsAction.ADMIN_PLAYER_FILTER) return
        val lang = plugin.languageManager
        val messageText =
                ...
                .runTask(
                        ...
                        Runnable {
                            ...
                            if (messageText.equals(cancelWord, ignoreCase = true)) {
                                ...
                                player.sendMessage(
                                        lang.getMessage(player, "messages.operation_cancelled")
                                )
                                return@Runnable
                            }
                            ...
                            when (session.action) {
                                ...
                                SettingsAction.CHANGE_DESCRIPTION -> {
                                    ...
                                    plugin.worldConfigRepository.save(worldData)
                                    player.sendMessage(
                                            lang.getMessage(player, "messages.world_desc_change")
                                    )
                                ...
                                SettingsAction.MEMBER_INVITE -> {
                                    ...
                                    if (target == null) {
                                        player.sendMessage(
                                                lang.getMessage(player, "general.player_not_found")
                                        )
                                        player.playSound(
                                                player.location,
                                ...
                                SettingsAction.SET_ANNOUNCEMENT -> {
                                    ...
                                    if (input.equals("exit", ignoreCase = true)) {
                                        plugin.worldConfigRepository.save(worldData)
                                        player.sendMessage(
                                                lang.getMessage(player, "messages.announcement_set")
                                        )
                                        plugin.settingsSessionManager.endSession(player)
                                        plugin.worldSettingsGui.open(player, worldData)
                                        return@Runnable
    ...
    fun onInteract(event: PlayerInteractEvent) {
        ...
        if (currentAction == SettingsAction.EXPAND_DIRECTION_WAIT) {
            if (event.action == Action.LEFT_CLICK_AIR || event.action == Action.LEFT_CLICK_BLOCK) {
                ...
                settingsSession.action = SettingsAction.EXPAND_CONFIRM

                val worldData = plugin.worldConfigRepository.findByUuid(settingsSession.worldUuid)
                val cost = calculateExpansionCost(worldData?.borderExpansionLevel ?: 0)

                val directionKey =
                        when (direction) {
                            BlockFace.NORTH_WEST -> "general.direction.north_west"
                            BlockFace.NORTH_EAST -> "general.direction.north_east"
                            BlockFace.SOUTH_WEST -> "general.direction.south_west"
                            BlockFace.SOUTH_EAST -> "general.direction.south_east"
        ...
        if (currentAction == SettingsAction.SET_SPAWN_GUEST ||
            ...
            if (worldData != null) {
                if (currentAction == SettingsAction.SET_SPAWN_GUEST) {
                    ...
                    player.sendMessage(
                            plugin.languageManager.getMessage("messages.spawn_guest_set")
                    )
                } else {
                    worldData.spawnPosMember = loc
                    player.sendMessage(
                            plugin.languageManager.getMessage("messages.spawn_member_set")
                    )
                }
                plugin.worldConfigRepository.save(worldData)
                plugin.settingsSessionManager.endSession(player)
    ...
    private fun executeBiomeChange(
        ...
        try {
            ...
            player.sendMessage(
                    lang.getMessage(player, "messages.env_cost_paid", mapOf("cost" to cost))
            )
    ...
    private fun removeFromInventory(player: Player, templateItem: org.bukkit.inventory.ItemStack) {
    ...
    private fun applyGravityToWorld(worldData: WorldData) {
        ...
        world.entities.forEach { entity ->
            if (entity is org.bukkit.entity.LivingEntity) {
                entity.getAttribute(org.bukkit.attribute.Attribute.GRAVITY)?.baseValue =
                        gravityValue
            }
        }
    }
    ...
    private fun applyBiomeToWorld(worldData: WorldData) {
        ...
        // Iterate loaded chunks instead of all blocks directly to avoid loading unloaded chunks if
        // possible,
        ...
        // If we want to ensure *all* blocks in range are changed (even unloaded), we should iterate
        // strict range.
        // However, iterating 60million blocks is bad.
        // But for a typical MyWorld with expansion 0-3, it's small.
        // Let's assume we only update loaded chunks because unloaded ones will be handled by
        // ChunkLoadEvent.
